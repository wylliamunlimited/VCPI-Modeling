...
def counts_to_expression(
    counts: pd.DataFrame, metadata: pd.DataFrame, genes: set[str] | None = None
) -> pd.DataFrame:
    ...
    depth = meta.loc[
        counts.columns, "total_umi_count"
    ].to_numpy()  ## filtered by gene_id
    compound = meta.loc[
        counts.columns, "user_compound_id"
    ].to_numpy()  ## filtered by gene_id
